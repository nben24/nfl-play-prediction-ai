"""
Tests for the rule-based reasoning engine in explainer.py.

These tests cover _build_reasoning() only — the deterministic Python layer
that generates bullet points. The LLM layer is not tested here because it
is non-deterministic and requires a running Ollama instance.
"""

from explainer import _build_reasoning
...
def test_midfield_no_field_position_point():
    ...
    field_points = [
        p for p in points
        if "zone" in p.lower() or "territory" in p.lower()
...
def test_game_winning_td_explicitly_says_not_tied():
    points = _build_reasoning(down=4, ydstogo=3, yardline_100=3,
                              score_diff=-2, qtr=4, seconds=24, prediction="pass")
    assert "not tied" in " ".join(points).lower()
...
def test_tied_early_no_urgency_point():
    # Q1 tied — no score/time pressure point should be added
    points = _build_reasoning(down=1, ydstogo=10, yardline_100=70,
                              score_diff=0, qtr=1, seconds=3400, prediction="run")
    score_time_points = [
        p for p in points
        if "tied" in p.lower() or "trailing" in p.lower()
